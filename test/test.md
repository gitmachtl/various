...
|waypoint `id` after `id1, ...idx`| Gibt an, nach welchem letzten aktivierten Wegpunkt `id1, ...idx` der Wegpunkt `id` freigeschaltet werden soll.<br>Das Keyword `every` kann verwendet werden, um damit alle möglichen IDs einzuschließen.|`waypoint 120 after 030,050`<br>`waypoint 000 after every`|
